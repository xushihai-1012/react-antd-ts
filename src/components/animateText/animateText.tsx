...
const defaultRun: boolean = true;
const infinite: boolean = true;
const frameTime: number = 75;
const endWaitStep = 3;
const prefixString = "";
let runTexts = [""];
const colorTextLength = 5;
const step = 1;
const colors = [
  "rgb(110,64,170)",
  "rgb(150,61,179)",
...
const randomTime: number = randomNum(15, 150);
...
const AnimateText = (props: { texts: any; }) => {
  const { texts } = props;
  ...
  function loop(): void {
    ...
    setTimeout(() => {
      if (continue2 && container.current != null) {
        ...
        const index = inst.skillI;
        const originText = texts[index];
        const currentText = runTexts[index];
